...
function App() {
  ...
  const getVeriationsDetails = (skus, singleProps) => {
    ...
    console.log(" getVeriationsDetails function calling ")
    const finnalModifiedResults = skus.map((singleSkus) => {

      const data = getVeriationsNameAndValue(singleSkus.props, singleProps)

      return { ...data, otherDetails: singleSkus };
    });
  ...
  const onValueChange = (e, sku, propsId) => {
    ...
    setData(pre => { return { ...modifiedData, updated: true } })
  ...
  return (
    <div style={{ width: "500px" }}>
      <ul className="App">
        ...
        <li>  Selete
          <ul>
            {data.skus.map(({ id }) => <button onClick={() => setSeleteData(pre => {
              ...
              <ul>
                {getVeriationsDetails(data.skus, x).map((i, inx) => {
                  ...
                  return <>
                    ...
                    <li style={{ cursor: "pointer" }} key={inx}>  <input onChange={(e) => onValueChange(e, i, x.id)} type="text" defaultValue={i?.title} />  </li>
                  </>
                })}
              </ul>
            </li>
        ...
        <li>
          ...
          <ul>
            {getVeriationsDetails(data.skus, data.props[1]).map((j, inx) => {
              ...
              return <li key={inx}>  <input type="text" defaultValue={j?.otherDetails.price.actual} />  </li>
            })
